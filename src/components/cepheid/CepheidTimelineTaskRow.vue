...
import { starAvatarProps, type TeamMember } from '../../composables/sidera/useTeamMembers'
...
<template>
  <div class="row">
    ...
    <div class="cell">
      <div class="task" :class="{ done: task.done, locked: !unlocked }">
        ...
        <div class="trow2">
          <div class="avg">
            <StarAvatar
              ...
              :key="email"
              class="av"
              v-bind="starAvatarProps(email, members)"
              :size="22"
            />
          </div>
...
.task.locked { opacity: .4; pointer-events: none; }
...
.spacer { flex: 1; }
.avg { display: flex; align-items: center; flex: 0 0 auto; }
.avg .av:not(:first-child) { margin-left: -7px; }
.avg .av { border: 1.5px solid var(--md-sys-color-surface); border-radius: 50%; }
