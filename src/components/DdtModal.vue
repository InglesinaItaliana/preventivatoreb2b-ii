<script setup lang="ts">
  ...
  // --- RIMOSSE LE VARIABILI DUPLICATE (dataDdt, numeroColli, pesoKg) ---
  ...
  watch(() => props.orders, (newOrders: any[]) => {
    if (newOrders.length > 0) {
      ...
      const totaleColli = newOrders.reduce((somma: number, ordine: any) => {
          // Se l'ordine ha un campo 'colli' lo usa, altrimenti conta come 1
          return somma + (ordine.colli ? Number(ordine.colli) : 1);
      }, 0);
  
      form.colli = totaleColli;
      // Reset peso (opzionale)
      form.weight = 0; 
    }
                    ...
                    <div class="mt-2">
                      ...
                      <div class="mb-4 bg-slate-50 p-3 rounded-xl border border-slate-200 mt-4">
                        ...
                        <div class="flex gap-2">
                          <button 
                            ...
                            :class="form.tipoTrasporto === 'INTERNAL' ? 'bg-amber-400 text-amber-950 border-amber-400' : 'bg-white text-slate-500 border-slate-200'"
                          >
                            Mezzi Interni
                          </button>
                          <button 
                      ...
                      <div class="mt-4 grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-2">
                          <div class="col-span-2 sm:col-span-1">
                              ...
                              <input type="date" v-model="form.date" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                          </div>
  
                          <div class="col-span-2 sm:col-span-1">
                              <label class="block text-xs font-bold text-gray-500 uppercase mb-1">N. Colli Totali</label>
                              <input type="number" v-model.number="form.colli" min="1" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                          </div>
                          ...
                          <div class="col-span-2">
                              ...
                              <input type="number" v-model.number="form.weight" min="0" step="0.1" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                          </div>
                      </div>
                      ...
                      <div class="mt-4 bg-gray-50 p-3 rounded text-xs text-gray-500 border border-gray-200">
                          <ul class="list-disc pl-4 space-y-1">
                              ...
                              <li>Trasporto a cura di: <strong>MITTENTE</strong> (o Vettore)</li>
