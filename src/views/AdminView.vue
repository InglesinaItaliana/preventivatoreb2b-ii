...
// Incolla questo blocco nello script di AdminView.vue
...
const getStatusStyling = (stato: string) => {
  const styles: Record<string, { badge: string; icon: any; iconBg: string; }> = {
    ...
    'IN_ATTESA': { badge: 'bg-orange-100 text-orange-700 border-orange-200', icon: ShieldExclamationIcon, iconBg: 'bg-orange-100 text-orange-600' }, // Alias per Admin
    'QUOTE_READY': { badge: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircleIcon, iconBg: 'bg-green-100 text-green-600' },
    'APPROVATO': { badge: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircleIcon, iconBg: 'bg-green-100 text-green-600' }, // Alias per Admin
    ...
    'RICHIESTA_ORDINE': { badge: 'bg-purple-100 text-purple-700 border-purple-200', icon: PaperAirplaneIcon, iconBg: 'bg-purple-100 text-purple-600' }, // Alias per Admin
...
const getStatusBadge = (stato: string) => {
 switch(stato) {
   ...
   case 'QUOTE_READY': case 'APPROVATO': return 'bg-green-50 text-green-600 border-green-100';
   ...
   <div class="max-w-7xl mx-auto">
     ...
     <div v-else-if="activeTab === 'COMMESSE'" class="space-y-4">
       <div v-for="gruppo in preventiviPerStato" :key="gruppo.stato" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div @click="toggleStato(gruppo.stato)" class="p-4 cursor-pointer hover:bg-gray-50 flex justify-between items-center border-l-4" :class="getStatusBadge(gruppo.stato).replace('text-','border-').split(' ')[0]">
              <div class="flex items-center gap-3">
                  <span class="font-bold font-heading text-sm uppercase" :class="getStatusBadge(gruppo.stato).split(' ')[1]">{{ getStatusLabel(gruppo.stato) }}</span>
                       ...
                       <div>
                        <div class="flex items-center gap-2">
                          <h3 class="font-bold text-gray-900">
                            {{ anagraficaClienti[p.clienteEmail] || p.cliente || 'Cliente Sconosciuto' }}
                          </h3>
                          <span class="text-xs text-gray-500">
                            ({{ p.commessa || 'Senza Nome' }})
                          </span>
                        </div>
