...
// IMPORT LOGICA PREZZI
import { calculatePrice } from '../logic/pricing'; 

const clienteEmail = ref('');

const showLegalModal = ref(false);
const legalCheck1 = ref(false); // Termini
const legalCheck2 = ref(false); // Clausole
...
// 2. Validazione (Curve o Note richiedono controllo umano)
...
// 3. BLOCCO MODIFICHE (IS LOCKED)
// Il preventivo è bloccato per il cliente se non è nello stato di BOZZA.
...
watch([copiaDuplex, tipoGriglia, dimensioneGriglia, finituraGriglia, categoriaGriglia], ([attivo, tipo, dim, fin, cat]) => {
    if (attivo && cat === 'DUPLEX') {
      tipoCanalino.value = tipo; dimensioneCanalino.value = dim; finituraCanalino.value = fin;
    }
...
watch(tipiGrigliaDisp, (v) => { if (v.length === 1) tipoGriglia.value = v[0]; });
watch(dimensioniGrigliaDisp, (v) => { if (v.length === 1) dimensioneGriglia.value = v[0]; });
watch(finitureGrigliaDisp, (v) => { if (v.length === 1) finituraGriglia.value = v[0]; });
watch(tipiCanalinoDisp, (v) => { if (v.length === 1 && !copiaDuplex.value) tipoCanalino.value = v[0]; });
...
const aggiungi = () => {
  ...
  preventivo.value.push({
    ...
    base_mm: pannello.base, altezza_mm: pannello.altezza, 
    righe: pannello.righe || 0, colonne: pannello.colonne || 0, 
...
const uploadFile = async (event: Event) => {
  ...
  try {
    ...
    listaAllegati.value.push({ nome: file.name, url: url, tipo: file.name.split('.').pop()?.toUpperCase() || 'FILE', data: new Date().toISOString() });
  } catch (e) { alert("Errore upload. Controlla permessi Storage."); console.error(e); } 
...
const salvaPreventivo = async (azione?: string) => {
  ...
  if (preventivo.value.length === 0) {
    alert("Il preventivo è vuoto. Aggiungi almeno un elemento.");
    return;
  }

  // 2. INTERCETTAZIONE ORDINE (Logica Legale)
  // Se l'azione è "Voglio Ordinare", non salviamo subito.
  // Apriamo la modale per la firma e l'accettazione termini.
  if (azione === 'RICHIESTA_ORDINE') {
      if (isAdmin.value) {
         alert("L'amministratore non può inviare ordini per conto del cliente.");
         return;
      }
      // Apri la modale e esci da questa funzione
  ...
  try {
    ...
    // Se stiamo solo aggiornando i dati (nessuna azione specifica) e non servono validazioni
    if (currentDocId.value && !azione && !richiedeValidazione.value) {
        nuovoStato = statoCorrente.value; 
    }

    // Se ci sono curve o note -> Va sempre in validazione (tranne se è già approvato/firmato)
    // (Nota: se è già QUOTE_READY e modifico aggiungendo una nota, torna PENDING_VAL)
    if (richiedeValidazione.value && statoCorrente.value !== 'SIGNED' && statoCorrente.value !== 'IN_PRODUZIONE') {
    ...
    // 4. PREPARAZIONE DATI (Schema Aggiornato)
    const docData = {
      codice,
      cliente: nomeCliente.value,       // "Vetreria Rossi" (Visuale)
      clienteEmail: clienteEmail.value, // "rossi@gmail.com" (Tecnico per query)
      ...
      allegati: listaAllegati.value, // Array di oggetti {nome, url, tipo}
      
      // Date
      dataModifica: serverTimestamp(), // Aggiorniamo la data di modifica
      // Se è nuovo, mettiamo anche la creazione
      ...(currentDocId.value ? {} : { dataCreazione: serverTimestamp() }),
      // Aggiorniamo la scadenza a 30gg da oggi
    ...
    if (currentDocId.value) {
      // Update esistente (merge: true per sicurezza, anche se qui sovrascriviamo tutto)
    ...
    let msg = `✅ Salvataggio completato. Stato: ${nuovoStato}`;
    if (nuovoStato === 'PENDING_VAL') msg = `⚠️ Preventivo inviato per validazione tecnica (Note/Curve).\nCodice: ${codice}`;
    if (nuovoStato === 'QUOTE_READY') msg = `🎉 Preventivo VALIDATO e pronto per l'ordine!`;
    
    alert(msg);
    
    // Routing post-salvataggio
    if (isAdmin.value) {
        router.push('/admin');
    } else {
        caricaListaStorico(); // Ricarica la sidebar
    }
...
const caricaListaStorico = async () => {
  try {
    // 1. Recupera l'utente loggato da Firebase Auth
    const user = auth.currentUser;
    
    // Sicurezza: Se non sei loggato, non vedi nulla
    if (!user) {
        storicoPreventivi.value = [];
        return;
    }
    ...
    if (isAdmin.value) {
        ...
        // --- VISTA CLIENTE: Filtra per la sua email ---
        // (Assicurati che il salvataggio ora scriva l'email nel campo 'cliente')
        q = query(
            collection(db, 'preventivi'), 
    ...
    // 3. Mappa i risultati per la lista visiva
    storicoPreventivi.value = snapshot.docs.map(doc => {
      const d = doc.data();
      return {
        id: doc.id,
        ...d,
        // Assicuriamo che ci sia uno stato valido (default: DRAFT)
        stato: d.stato || 'DRAFT',
        // Formattiamo la data per renderla leggibile
        dataLeggibile: d.dataCreazione ? new Date(d.dataCreazione.seconds * 1000).toLocaleDateString() : 'N/D'
      };
    });

  } catch (e) {
    console.warn("Errore caricamento storico:", e);
    // Se vedi in console un errore rosso con un link, cliccaci per creare l'indice su Firebase!
  }
};

const modificaRiga = async (index: number) => {
    if (isLocked.value) return; // Sicurezza extra
...
const confermaOrdineLegale = async () => {
  if (!legalCheck1.value || !legalCheck2.value) {
    alert("Devi accettare tutte le condizioni per procedere.");
    return;
  }
  ...
  try {
    // 1. Recuperiamo l'IP del cliente (servizio esterno gratuito)
    const ipRes = await fetch('https://api.ipify.org?format=json');
    const ipData = await ipRes.json();

    // 2. Dati Utente Reali (da Auth)
    const user = auth.currentUser;
    if (!user) throw new Error("Utente non autenticato");

    // 3. Creiamo il pacchetto "Audit Trail"
    const datiLegali = {
      accettazioneTermini: true,
      accettazioneClausole: true,
      ipCliente: ipData.ip,
      userAgent: navigator.userAgent,
      dataFirma: new Date().toISOString(), // ISO string per leggibilità
      firmatarioEmail: user.email,
      firmatarioUid: user.uid
    };

    // 4. Aggiorniamo il documento con STATO 'ORDER_REQ' e DATI LEGALI
    // (Nota: Passiamo a ORDER_REQ perché l'admin deve comunque validare/controllare prima di chiudere)
    // Oppure se vuoi che diventi SUBITO 'SIGNED' perché è già validato:
    const nextState = 'ORDER_REQ'; // O 'SIGNED' se < 5000? Tu hai detto "l'admin controlla SEMPRE"
    ...
    if (currentDocId.value) {
      await setDoc(doc(db, 'preventivi', currentDocId.value), updateData, { merge: true });
    }

    // 5. TRIGGER MAIL (Simulato per ora - qui chiameremmo la Cloud Function)
    console.log("TRIGGER MAIL A:", user.email, "CON PDF RIEPILOGO");
    alert(`✅ Ordine Confermato Legalmente!\nAbbiamo inviato una mail di ricevuta a ${user.email}.`);

    showLegalModal.value = false;
    statoCorrente.value = nextState;
    // router.push('/dashboard'); // Opzionale: rimanda alla home

  } catch (e) {
    console.error(e);
    alert("Errore durante la firma digitale.");
  } finally {
...
onMounted(() => {
  ...
  if (storedName) nomeCliente.value = storedName; // "Vetreria Rossi"
  // RECUPERA UTENTE REALE
  onAuthStateChanged(auth, (user) => {
      if (user && user.email) {
          // Se sto creando un NUOVO preventivo, imposto il mio nome
          if (!currentDocId.value) {
              nomeCliente.value = user.email; 
          }
          caricaListaStorico(); // Carica storico filtrato
      }
  });

  // ... resto del codice codiceUrl ...
});
</script>
                ...
                <template v-if="!isAdmin">
                    ...
                    <div v-else class="flex gap-2">
                        ...
                        <button 
                            ...
                            class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-bold shadow-lg shadow-purple-900/50 flex items-center gap-2 transition-transform active:scale-95"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                            ORDINA SUBITO
                        </button>
                        ...
                        <button 
                            ...
                            :disabled="preventivo.length === 0"
                            class="bg-yellow-400 hover:bg-yellow-500 text-black px-8 py-3 rounded-lg font-bold shadow-lg shadow-yellow-400/20 transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
      ...
      <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in-up">
        ...
        <div class="p-6 space-y-4">
           <p class="text-sm text-gray-600">
             Stai per confermare l'ordine <strong>{{ codiceRicerca || 'BOZZA' }}</strong> per un totale di <strong>{{ totaleFinale.toFixed(2) }} €</strong>.
             Questa azione ha valore legale di impegno all'acquisto.
           </p>

           <div class="bg-gray-50 p-3 rounded border border-gray-200 text-xs text-gray-500 font-mono">
             👤 Firmatario: {{ auth.currentUser?.email }}<br>
             📅 Data: {{ new Date().toLocaleDateString() }}
           </div>

           <div class="space-y-3 mt-4">
              <label class="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" v-model="legalCheck1" class="mt-1 h-4 w-4 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500">
                <span class="text-sm text-gray-800">Accetto l'ordine come descritto nel riepilogo e confermo la correttezza delle misure inserite.</span>
              </label>

              <label class="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" v-model="legalCheck2" class="mt-1 h-4 w-4 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500">
                <span class="text-sm text-gray-800">Dichiaro di aver letto e accettato le Condizioni Generali di Vendita e le clausole vessatorie (artt. 1341 e 1342 c.c.).</span>
              </label>
           </div>
        </div>
        ...
        <div class="p-4 bg-gray-50 border-t flex justify-end gap-3">
           ...
           <button 
             @click="confermaOrdineLegale" 
             :disabled="!legalCheck1 || !legalCheck2 || isConfirming"
             class="px-6 py-2 bg-black text-white font-bold rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
           >
             ...
             {{ isConfirming ? 'FIRMA IN CORSO...' : 'CONFERMA E FIRMA' }}
           </button>
        </div>
      </div>
    </div>
