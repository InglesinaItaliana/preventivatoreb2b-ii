<script setup lang="ts">
  ...
  import NovitaPanel from '../components/novita/NovitaPanel.vue';
  ...
  // --- STATO PER IL TUTORIAL ---
  const tourCompleted = ref(false); // Letto da Firebase
  const profileLoaded = ref(false); // Per sincronizzare profilo e dati
  const tourAlreadyChecked = ref(false); // Per evitare loop
...
const confermaRicezione = async (order: any) => {
  ...
  // 2. AGGIORNAMENTO VISTA E TRIGGER TOUR
  const aggiornaVista = () => {
    ...
    if (ready1 && ready2) {
      loading.value = false;
      checkAndStartTour(); // Riprova a lanciare il tour ora che i dati sono pronti
    }
  };
  ...
  const checkAndStartTour = async () => {
    // Avvia SOLO se:
    // 1. Il caricamento dati preventivi è finito (!loading)
    // 2. Il profilo utente è caricato (profileLoaded)
    // 3. L'utente NON ha mai fatto il tour (!tourCompleted)
    // 4. Non abbiamo già provato a lanciarlo in questa sessione (!tourAlreadyChecked)
    if (!loading.value && profileLoaded.value && !tourCompleted.value && !tourAlreadyChecked.value) {
      tourAlreadyChecked.value = true; // Evita loop
      
      // Aspetta che il DOM sia renderizzato (es. il bottone "Inizia ora" potrebbe apparire dopo)
      await nextTick();
      startOnboardingTour();
    }
  };
  
  let dashboardTour: TourGuideClient | null = null;

 const startOnboardingTour = () => {
  // 1. Creiamo l'istanza una sola volta (Singleton)
  if (!dashboardTour) {
    dashboardTour = new TourGuideClient({
      exitOnClickOutside: false,
      closeButton: false,
      backdropColor: 'rgba(15, 23, 42, 0.85)',
      completeOnFinish: false,
      rememberStep: false, // <--- FONDAMENTALE: Resetta il tour ogni volta
      steps: [] // Li definiamo dinamicamente sotto
    });

    // Callback di fine tour (agganciata una volta sola)
    dashboardTour.onFinish(async () => {
      const user = auth.currentUser;
      if (user) {
        try {
          await updateDoc(doc(db, 'users', user.uid), { tourCompleted: true });
          tourCompleted.value = true;
        } catch (e) { console.error("Errore salvataggio tour:", e); }
      }
    });
  }

  // 2. Aggiorniamo gli step ogni volta (per gestire il caso Lista Vuota vs Piena)
  const currentSteps = [
    {
      target: '#tour-welcome',
      title: 'La tua Dashboard',
      content: `Ciao <b>${clientName.value}</b>!<br>Questa è la tua area operativa per gestire preventivi e ordini.`,
    },
    {
      target: '#tour-tabs',
      title: 'Il Ciclo di Lavoro',
      content: 'I tuoi progetti si sposteranno automaticamente attraverso queste fasi: da <b>Preventivi</b> a <b>Ordini</b>, poi in <b>Produzione</b> e infine <b>Spedizioni</b>.',
    },
    // STEP DINAMICO (ricalcolato al momento del click)
    preventiviInCorso.value.length === 0 
    ? {
        target: '#tour-start-empty',
        title: 'Inizia da qui',
        content: 'Clicca su <b>"Inizia ora"</b> per configurare il tuo primo Preventivo / Ordine.',
      }
    : {
        target: '#tour-list-container',
        title: 'I tuoi Preventivi',
        content: 'Qui trovi le bozze che hai già creato. Clicca su "Apri" per modificarle o inviarle.',
      },
    // STEP FAB (Aggiunto recentemente)
    {
      target: '#global-fab-btn',
      title: 'Azioni Rapide',
      content: 'Clicca qui in qualsiasi momento per <b>creare una nuova commessa</b> al volo o per <b>segnalare un problema</b> tecnico al nostro supporto.',
    },
    {
      target: '#tour-archive',
      title: 'Archivio Storico',
      content: 'Tutti i tuoi lavori conclusi verranno salvati qui per consultazione futura.',
    }
  ];

  // Imposta gli step aggiornati e avvia
  dashboardTour.setOptions({ steps: currentSteps });
  dashboardTour.start();
};
  
  const avviaAscoltoDati = (uid: string) => {
    loading.value = true;
    ready1 = false;
  ...
  // Il popup one-time della funzione "stampa documento" è stato ritirato: la
  // novità vive nel pannello (lib/novita.ts, 'stampa-documenti-v1'), dove resta
  // consultabile invece di sparire dopo il primo "Ho capito". Della vecchia
  // scheda è rimasta la sola idea utile — far VEDERE il pulsante — nel campo
  // `pulsante` della voce di registro.
  ...
  if (unsub1) unsub1();   
  if (dashboardTour) {
    try { dashboardTour.exit(); } catch(e) {}
  }
});
  </script>
      ...
      <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start mb-12 gap-4">
          ...
          <div class="flex items-center gap-3 mt-4 md:mt-0">
            <NovitaPanel id="tour-novita" />
            <button
              @click="startOnboardingTour"
              class="bg-white hover:bg-gray-50 border border-gray-200 text-gray-600 px-3 py-2 rounded-full font-bold shadow-sm flex items-center gap-2 transition-transform active:scale-95 text-xs"
            >
                <QuestionMarkCircleIcon class="h-5 w-5 text-amber-500" /> GUIDA
            </button>
            <button id="tour-archive" @click="showArchive = true" class="bg-white hover:bg-gray-50 border border-gray-200 text-gray-600 px-3 py-2 rounded-full font-bold shadow-sm flex items-center gap-2 transition-transform active:scale-95 text-xs">
                <ArchiveBoxIcon class="h-5 w-5 text-gray-600" /> ARCHIVIO
            </button>
          </div>
        </div>
  
        <div id="tour-tabs" class="flex overflow-x-auto mb-4 gap-2 p-4 -ml-4">        
        ...
        <div v-else>
          ...
          <div id="tour-list-container" v-if="activeTab === 'PREVENTIVI'" class="grid gap-4">
            <div v-if="preventiviInCorso.length === 0" class="text-center py-20 bg-white/50 backdrop-blur-sm backdrop-saturate-150 rounded-[2rem] shadow-lg border border-white/80">
              <p class="text-gray-500 mb-4">Non hai preventivi aperti al momento.</p>
              <button id="tour-start-empty" @click="vaiAlBuilder()" class="text-amber-950 font-bold underline">Inizia ora</button>
            </div>
          ...
          <div v-if="activeTab === 'ORDINI'" class="grid gap-4">
            <div v-if="ordiniConfermati.length === 0" class="text-center py-20 bg-white/50 backdrop-blur-sm backdrop-saturate-150 rounded-[2rem] shadow-lg border border-white/80">
          ...
          <div v-if="activeTab === 'PRODUZIONE'" class="grid gap-4">
            <div v-if="ordiniInProduzione.length === 0" class="text-center py-20 bg-white/50 backdrop-blur-sm backdrop-saturate-150 rounded-[2rem] shadow-lg border border-white/80">
          ...
          <div v-if="activeTab === 'SPEDIZIONI'" class="grid gap-4">
            <div v-if="ordiniSpediti.length === 0" class="text-center py-20 bg-white/50 backdrop-blur-sm backdrop-saturate-150 rounded-[2rem] shadow-lg border border-white/80">
