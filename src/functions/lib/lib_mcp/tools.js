...
async function linkDeliverable(args, ctx) {
    ...
    if (!dSnap.exists)
        throw new Error(`Deliverable ${args.deliverableId} non trovato in projects/${args.projectId}/tasks. Verifica projectId/deliverableId con cepheid_getProject.`);
    if (((_a = dSnap.data()) === null || _a === void 0 ? void 0 : _a.type) !== 'deliverable')
        throw new Error(`L'entità ${args.deliverableId} non è un deliverable (type=${(_b = dSnap.data()) === null || _b === void 0 ? void 0 : _b.type}). Usa nebula_linkTask o nebula_linkMilestone secondo il tipo.`);
...
async function linkMilestone(args, ctx) {
    ...
    if (!mSnap.exists)
        throw new Error(`Milestone ${args.milestoneId} non trovata in projects/${args.projectId}/tasks. Verifica projectId/milestoneId con cepheid_getProject.`);
    if (((_a = mSnap.data()) === null || _a === void 0 ? void 0 : _a.type) !== 'milestone')
        throw new Error(`L'entità ${args.milestoneId} non è una milestone (type=${(_b = mSnap.data()) === null || _b === void 0 ? void 0 : _b.type}). Usa nebula_linkTask o nebula_linkDeliverable secondo il tipo.`);
...
async function linkObiettivo(args, ctx) {
    ...
    if (!oSnap.exists)
        throw new Error(`Obiettivo ${args.obiettivoId} non trovato in obiettivi/. Verifica l'id con cepheid_listObiettivi.`);
    const out = await appendMentionParagraph(args.docId, { type: 'obiettivoMention', attrs: { obiettivoId: args.obiettivoId, title: args.title || ((_a = oSnap.data()) === null || _a === void 0 ? void 0 : _a.titolo) || '' } }, ctx.userEmail);
