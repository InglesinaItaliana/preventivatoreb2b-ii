...
import { createCicProvider, getActiveBackend, buildDdtLines, isRigaConsegna, round2, fetchCompanyInfo, diffCompany, destinazioneComune, hasDestinazione, formatDestinazione, type DestinazioneMerce } from './lib_billing';
 ...
 */
export const backfillAssigneeUids = functions
  .region('europe-west1')
  .https.onCall(async (data, context) => {
    const callerEmail = (context.auth?.token?.email || '').toLowerCase().trim();
    const callerRole = context.auth?.token?.role;
    ...
    for (const taskDoc of tasksSnap.docs) {
      tasksScanned++;
      const a = taskDoc.data()?.assignees;
      if (!Array.isArray(a) || a.length === 0) continue;
      let changed = false;
      const next = a.map((raw: unknown) => {
        if (typeof raw !== 'string' || !raw) return raw;
        const v = raw.toLowerCase().trim();
        if (v.includes('@')) {
          const uid = emailToUid.get(v);
          if (uid) { changed = true; refsConverted++; return uid; }
          refsUnresolved++; unresolvedSamples.add(v); return raw;   // orfano: resta email
        }
...
async function creaDdtCumulativoCiC(
    ...
    try {
        ...
        const esitoDest = destinazioneComune(orders.map((o) => o.data.destinazione));
        if (!esitoDest.ok) return { success: false, message: esitoDest.errore };
        const destinazione: DestinazioneMerce | undefined = esitoDest.destinazione;
        ...
        const lines: any[] = buildDdtLines(
            orders.map((o) => o.data as any),
            scontoPagamento,
            tipoTrasporto === 'COURIER' ? 'COURIER' : 'INTERNAL',
        );
        ...
        if (destinazione) {
            console.log(`📍 [CIC DDT] Destinazione alternativa: ${formatDestinazione(destinazione)}`);
        }
        ...
        const nettoAtteso = round2(lines
            .filter((l: any) => !l.isDescriptive)
            .reduce((acc: number, l: any) =>
                acc + round2(l.qty * l.unitNetPrice * (1 - (l.discountPercentage || 0) / 100)), 0));
        ...
        const erroreDelDdt = (msg: unknown) => typeof msg === 'string'
        ...
        for (const o of orders) {
            ...
            if (nettoDivergente) {
                patch.billingError = `DDT ${ddt.number ?? ddt.id}: netto CiC ${ddt.netAmount} ≠ atteso POPS ${nettoAtteso} — verificare prima della fatturazione`;
...
exports.creaDdtCumulativo = functions
    ...
    .https.onCall(async (data, _context) => {
        ...
        try {
            ...
            if (allSnaps.some((s) => hasDestinazione(s.data()?.destinazione))) {
                return {
                    success: false,
...
exports.manageCustomerDiscount = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        const piva = String(data?.piva || '').trim().replace(/^IT/i, '');
        if (!piva) throw new functions.https.HttpsError('invalid-argument', 'P.IVA mancante.');
        ...
        try {
            ...
            if (!cliente) {
                throw new functions.https.HttpsError(
                    'not-found',
                    `Cliente con P.IVA ${piva} non trovato su Contabilità in Cloud.`,
                );
            }
            ...
            console.error('❌ [CIC] manageCustomerDiscount:', e?.message || e);
...
exports.cercaCommessaArchivio = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        const termine = String(data?.termine || '').trim().toUpperCase();
        if (termine.length < 2) {
        ...
        try {
            ...
            const trovati = snap.docs
                ...
                .filter((o: any) => String(o.commessa || '').toUpperCase().includes(termine));
            ...
            trovati.sort((a: any, b: any) => quando(b) - quando(a));
