 ...
 */
export const PROVINCE_CIC: Readonly<Record<string, number>> = Object.freeze({
...
export function normalizzaProvincia(v: unknown): string | undefined {
  ...
  return canonica in PROVINCE_CIC ? canonica : undefined;
}

/** provinceNumber Reviso per una sigla, o undefined se la sigla non è valida. */
export function provinciaNumber(v: unknown): number | undefined {
  const sigla = normalizzaProvincia(v);
  return sigla ? PROVINCE_CIC[sigla] : undefined;
}
 ...
 */
export function buildDestinationBlock(d: Partial<DestinazioneMerce> | null | undefined) {
  if (!hasDestinazione(d)) return null;
  ...
  return {
    ...
    country: 'Italia',
    countryCode: {
      ...(num ? { province: { id: num, metaData: null } } : {}),
      id: 'IT',
      metaData: null,
    },
    id: null,
