...
- `logic/priceBreakdown.ts` — `costruisciDettaglio()`: spiega/riconcilia il prezzo salvato senza mai ricalcolarlo come fonte di verità (`riconcilia:false` se non riproduce). Due strade: dallo scontrino quando la riga ce l'ha, per deduzione sulle righe storiche.
