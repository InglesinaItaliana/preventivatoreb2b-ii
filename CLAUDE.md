...
- `logic/listini.ts` — puro TS condiviso dai tre motori: anagrafica listini (`nomeListino`; `stessoListino` — `'2025x'` e `'2025-x'` sono lo STESSO listino LEALI) e **la formula unica dello scontrino** `metri × Σtariffe × (1+maggiorazione) + Σsupplementi` (`ricostruisciPrezzoUnitario`), più `pricingSoloTelaio()` e `regimeDaComplessita()`. Sta qui e non in `pricing.ts` per non creare un import circolare dispatcher↔motori 2025.
- **Scontrino di riga**: ogni riga si porta dietro `pricing?: RigaPricing` (`src/types.ts`) scritto dal motore che l'ha calcolata — assente sulle righe storiche. Test anti-drift `logic/__tests__/pricingSnapshot.test.ts`: su tutta la matrice listini×misure×suddivisioni×canalini lo scontrino deve riprodurre il prezzo del motore, altrimenti la lente mostra al cliente una scomposizione che non torna.
- `Data/catalog.ts` — store Pinia: `listino_base` (48 prezzi base, source of truth) + `catalogo` (~389 righe struttura menu che ereditano prezzo da `listino_base[cod]`); sorgente selezionata da `settings/pricing.catalogSource==='firestore'`, altrimenti fallback CSV Google Sheet. Espone `listino` (albero menu), `codiciMap` (cod→prezzo) e `supplementiMap` (cod→`{nome, casistica}`: i due testi che la lente del prezzo mostra sotto una voce fissa). ⚠️ In `listino_base` nessuno dei due campi si chiama come ci si aspetta — nome in **`modello`**, casistica in **`finitura`**; `dimensione` lì è il tipo canalino (AL/BC/FI). Dal foglio arriva la sola casistica (colonna DIMENSIONE), il nome ricade su `NOMI_VOCE` in `priceBreakdown.ts` che va tenuto uguale ai `modello`; `catalogo` ha copie che di proposito NON alimentano l'indice. Leggere il campo sbagliato non rompe niente e non fallisce i test di prezzo: cambia solo la frase mostrata al cliente. Guardie in `Data/__tests__/catalog.test.ts`.
- `logic/priceBreakdown.ts` — `costruisciDettaglio()`: spiega/riconcilia il prezzo salvato senza mai ricalcolarlo come fonte di verità (`riconcilia:false` se non riproduce). Due strade: dallo scontrino quando la riga ce l'ha, per deduzione sulle righe storiche. Il testo del blocco ③ è composto — discriminante fissa per regime + **meccanismo derivato dai numeri della riga** (c'è una percentuale? ci sono voci fisse?), così non può mentire sul conto che gli sta sotto: nel 2026 parallele e singola NON prendono maggiorazioni (canalino a forfait come materiale perimetrale + allestimento telaio), nei listini 2025 sì. Le maggiorazioni si mostrano in percentuale (+20%/+50%), mai come ×1,2/×1,5. Ogni voce fissa dichiara la **casistica** in cui è caduta la riga (`supplementiMap`, v. sopra): nome e soglia si leggono dal listino e non si ricopiano mai nel codice.
...
- **Destinazione merce** (luogo di consegna ≠ indirizzo cliente): campo `destinazione` sul preventivo (COPIA, non riferimento) + rubrica su `users/{uid}.destinazioni[]`. Regole pure in `lib_billing/destinazione.ts` con **specchio client** `src/lib/destinazione.ts` (il frontend non può importare da src/functions) e test anti-drift `src/lib/__tests__/destinazione.test.ts`. `destinazioneComune()` è autoritativa: **un DDT ha un solo luogo di consegna**, i mix vanno rifiutati (AdminView non li seleziona, la callable li blocca). Il path FiC non supporta il campo e rifiuta. Tabella province congelata: riverificabile con `node risorsexCiC/verifica-province.mjs`.
- **Il layout DDT stampa i blocchi del DOCUMENTO, non il master cliente**: `owner` (DESTINATARIO) e `destination` (LUOGO DI DESTINAZIONE) vanno riempiti nel payload o il DDT esce col solo nome, anche se su Reviso l'anagrafica è completa. `owner` lo costruisce `buildOwnerBlock()` (cicProvider.ts) dall'anagrafica che `mapCustomerRef()` porta dietro. Trappole: la provincia è un NUMERO Reviso dentro `countryCode.province.id` (Milano 58, non 'MI'), Reviso non conosce la sigla `SU` (Sud Sardegna = `SD`, 111) e `GET /provinces/IT` senza `?pagesize=1000` ne torna 20 su 112; `country` vuoto → il PDF stampa "undefined".
...
|---|---|---|
| `preventivi` | Doc unico preventivo→ordine→DDT | `stato`, `clienteUID`, `elementi[]`, `totaleImponibile/totaleScontato/netCanonico`, `commessa`, billing `billingBackend/billingError/fic_*/cic_*`, spedizione `colli/corriere/trackingCode`, `destinazione` (luogo di consegna alternativo), `listino` (motore prezzi congelato alla prima creazione; assente sui documenti storici) |
| `users` | Clienti B2B (doc id = Auth UID) | anagrafica (`ragioneSociale`, `piva` → lookup cliente FiC/CiC, `citta`) + pricing (`price_list_mode`, `delivery_tariff_code`, `detraction_value`) + `destinazioni[]` (rubrica indirizzi di consegna del cliente) |
